...
Ignore the time you need to serve the coffee and the time you need to take the orders and write down the time you need to make
 each one of them.

If you have three customers with times [4,3,2], the first customer is going to wait 4 minutes for his coffee, the second 
customer is going to wait 4 minutes (the time needed for the first customer to get his coffee), another 2 minutes 
(the time needed to clean the machine) and 3 more minutes (the time you need to brew his coffee), so in total 9 minutes.
 The third customer, by the same logic, is about to wait 9 minutes (for the first two customers) + 2 more minutes(cleaning) 
 + 2 minutes (his coffee brewing time). This order of brewing the coffee will end up in a total waiting time of 26 minutes,
...
function barista(coffees) {
  sortDescending(coffees);

  let totalWaitingTime = 0;
  for(let i = 0; i < coffees.length; i++){
   totalWaitingTime += coffees[i] * (coffees.length - i - 1);
...
let coffees1 = [3, 2, 5, 10, 9];
let coffees2 = [20, 5];
let coffees3 = [4, 3, 2];

sortDescending(coffees1);
sortDescending(coffees2);
sortDescending(coffees3);

console.log(barista(coffees1));
console.log(barista(coffees2));
console.log(barista(coffees3));
